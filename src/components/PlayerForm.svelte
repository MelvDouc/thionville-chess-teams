<script lang="ts">
  ...
  function selectAction(element: HTMLSelectElement) {
    element.value = String(player?.role ?? PlayerRole.USER);
  }
</script>
        ...
        <article class="col-12 col-sm-6">
          ...
          <div class="flex align-items-center">
            ...
            <FormCheckbox
              ...
              name="isMale"
              value="0"
              checked={player?.isMale === false}>♀️</FormCheckbox
            >
          </div>
        </article>
        <article class="col-12 col-sm-6">
          <label for="role" class="form-label form-required">Rôle</label>
          <select id="role" name="role" class="form-select" use:selectAction>
